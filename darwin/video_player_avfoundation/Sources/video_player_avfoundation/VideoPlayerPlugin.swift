...
public final class VideoPlayerPlugin: NSObject, FlutterPlugin, AVFoundationVideoPlayerApi {
  ...
  private var equalizerChannel: FlutterMethodChannel?
  private var equalizerEnabled = false
  private var equalizerBands: [Double] = Array(repeating: 0, count: 10)
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let equalizerChannel = FlutterMethodChannel(
      name: "tunevibe/video_player_eq",
      binaryMessenger: messenger
    )
    equalizerChannel.setMethodCallHandler { [weak instance] call, result in
      instance?.handleEqualizerMethodCall(call, result: result)
    }
    instance.equalizerChannel = equalizerChannel
  }
  ...
  private func configurePlayer(
    ...
    player.setEqualizerBands(equalizerBands.map { NSNumber(value: $0) })
  ...
  private func handleEqualizerMethodCall(_ call: FlutterMethodCall, result: FlutterResult) {
    switch call.method {
    case "setEnabled":
      let args = call.arguments as? [String: Any]
      equalizerEnabled = args?["enabled"] as? Bool ?? true
      for player in playersByIdentifier.values {
        player.setEqualizerEnabled(equalizerEnabled)
      }
      result(true)
    case "applyBands":
      let args = call.arguments as? [String: Any]
      if let gains = args?["gains"] as? [Any] {
        equalizerBands = gains.compactMap { value in
